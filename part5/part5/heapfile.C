...
HeapFile::HeapFile(const string & fileName, Status& returnStatus)
{
    ...
    if (status!=OK)
        cout << "open file error when running heapfile constructor" <<endl;

    status = filePtr->getFirstPage(headerPageNo);
    if (status!=OK)
        cout << "get first page number error when running heapfile constructor(getFirstPage) <<endl";

    status = bufMgr->readPage(filePtr, headerPageNo, pagePtr);
    if (status!=OK)
        cout << "read header page error when running heapfile constructor(readPage) <<endl";
    ...
    if (status!=OK)
        cout << "read first data page error when running heapfile constructor(readPage) <<endl";
...
HeapFile::~HeapFile()
{
    ...
    if (status!=OK) 
        cout << "Unpin hearder page error when running heapfile destructor" << endl;
    ...
    if (curPage != NULL)
    {
        ...
        curPageNo = 0;
        if (status != OK) 
            cout << "Unpin current page error when running heapfile destructor" << endl;
    }

/*
    status = bufMgr->flushFile(filePtr);
    if (status!=OK)
        cout << "flush file error when running heapfile destructor" << endl;
        //error.print(status);
*/
    status = db.closeFile(filePtr);
    if (status!=OK)
        cout << "close file error when running heapfile destructor" << endl;
}
